<table class="table table-hover table-sm tabel-border">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{ $row->name }}</td>
            <td>
                @if(file_exists('storage/' . $row->photo) && !empty($row->photo))
                <img src="{{ url('storage/'.$row->photo) }}" alt="{{ $row->image_alt }}" style="width: 84px;height: 48px;">
                @endif
            </td>
            <td>{{ $row->position }}</td>
            ...
            success: function(results) {
                if (results.status == "success") {
                    Swal.fire({
                        icon: 'success',
                        title: 'Success!',
                        text: "Gallery Status Successfully Changed !."
                    });
                } else {
                    Swal.fire({
                        icon: 'danger',
                        title: 'Danger!',
                        text: "Gallery Status Not Changed !."
                    });
                }
            }
        });
